...
class Http2TransportBackup extends DataTransport {
  ...
  #streamDataListener = (chunk) => {
    ...
    this.#internalBuffer = this.#internalBuffer
      ? Buffer.concat([this.#internalBuffer, chunk])
      : chunk;
  ...
  constructor(http2Stream, incomingHeaders) {
    ...
    this.#incomingHeaders = Object.fromEntries(
      incomingHeaders.entries
        ? incomingHeaders.entries()
        : Object.getEntries(incomingHeaders)
    );
  }
  ...
  startProcessingInput(
    useInternalBuffer,
    doProcessingAtEnd = false,
    clearOnDataObject = true
  ) {
  ...
  respond(headers, responseOptions) {
    ...
    if (
      !["application/json", "text/html", "application/cbor"].includes(
